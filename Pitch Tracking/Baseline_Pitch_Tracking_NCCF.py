...
def get_f0_from_nccf(r, n, fs):
    ...
    # peakind_acf, _ = signal.find_peaks(r_inter, distance=45)
    # highPeakind_acf = np.argmax(r_inter[peakind_acf])
    # t0_acf = peakind_acf[highPeakind_acf]    
...
def track_pitch_nccf(x,blockSize,hopSize,fs, thresholdDb): 
    ...
    masked_f0 = apply_voicing_mask(f0, mask)
...
def run_on_chunk(x,initPos,endPos,fs):
    f0, timeInSec = track_pitch_nccf(x[initPos:endPos], 1024, 512, fs, -40)
    return f0
